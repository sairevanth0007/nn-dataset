...
chromosome = {'conv1_filters': 96, 'conv1_kernel': 3, 'conv1_stride': 1, 'conv2_filters': 64, 'conv2_kernel': 3, 'conv3_filters': 384, 'conv4_filters': 512, 'conv5_filters': 512, 'fc1_neurons': 4096, 'fc2_neurons': 1024}
...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        super().__init__()
        self.device = device
        in_channels = in_shape[0] # Corrected for image channels

        self.features = nn.Sequential(
            # Layer 1
            nn.Conv2d(in_channels, 96, kernel_size=3, stride=1, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2), # Adjusted for CIFAR-10's small image size

            # Layer 2
            nn.Conv2d(96, 64, kernel_size=3, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2), # Adjusted for CIFAR-10's small image size

            # Layer 3
            nn.Conv2d(64, 384, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
        ...
        classifier_input_features = 512 * 6 * 6
